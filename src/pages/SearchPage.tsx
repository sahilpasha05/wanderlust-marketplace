  ...
  const filtered = useMemo(() => {
    return listings.filter((l) => {
      if (query && !l.title.toLowerCase().includes(query.toLowerCase()) && !l.location.toLowerCase().includes(query.toLowerCase()) && !l.country?.toLowerCase().includes(query.toLowerCase()))
        return false;
      const price = l.price_per_night;
      if (price < priceRange[0] || price > priceRange[1]) return false;
      if (selectedCategory && l.category !== selectedCategory) return false;
      if (minRating && (l.rating ?? 0) < minRating) return false;
      if (selectedAmenities.length > 0 && !(l.amenities ?? []).every((a) => selectedAmenities.includes(a))) return false;
      return true;
    });
  }, [listings, query, priceRange, selectedCategory, selectedAmenities, minRating]);
